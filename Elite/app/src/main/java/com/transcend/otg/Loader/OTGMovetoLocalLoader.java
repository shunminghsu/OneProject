 ...
 */
public class OTGMovetoLocalLoader extends AsyncTaskLoader<Boolean> {

    private static final String TAG = OTGMovetoLocalLoader.class.getSimpleName();
    ...
    private boolean Move() throws IOException {
        ...
        for (DocumentFile file : mSrcDocumentFileList) {
            if (file.isDirectory()) {
                moveDirectoryTask(mActivity, file, mDesFile);
            } else {
                moveFileTask(mActivity, file, mDesFile);
            }
        }
    ...
    private void moveDirectoryTask(Context context, DocumentFile srcFileItem, String destFileItem) throws IOException {
        ...
        for (DocumentFile file : files) {
            if (file.isDirectory()) {
                moveDirectoryTask(mActivity, file, destDirectory.getAbsolutePath());
            } else {//is file
                moveFileTask(mActivity, file, destDirectory.getAbsolutePath());
            }
        }
        srcFileItem.delete();
    }
    ...
    private void moveFileTask(Context context, DocumentFile srcFileItem, String destFileItem) throws IOException {
    ...
    public boolean moveFile(Context context, DocumentFile srcFileItem, File destFileItem) {
