 ...
 */
public class LocalBackuptoOTGLoader extends AsyncTaskLoader<Boolean> {

    private static final String TAG = LocalBackuptoOTGLoader.class.getSimpleName();
    ...
    private void createBackupFolder(){
        String folderName = mActivity.getResources().getString(R.string.backup_folder);
        if(mDesDocumentFile.findFile(folderName) == null)
            mDesDocumentFile = mDesDocumentFile.createDirectory(folderName);
        else
            mDesDocumentFile = mDesDocumentFile.findFile(folderName);
        if(mDesDocumentFile.findFile(mType) == null)
            mDesDocumentFile = mDesDocumentFile.createDirectory(mType);
        else
            mDesDocumentFile = mDesDocumentFile.findFile(mType);
